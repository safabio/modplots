...
mDotPlot2 <- function(
  ...
  pgenes=data.plot$features.plot ###

  if (!is.null(gnames)) { ####
    pgenes = gnames[gnames[,1] %in% data.plot$features.plot,] ###

    pgenes[,2]=as.character(pgenes[,2]) ###
    pgenes[,1]=as.character(pgenes[,1]) ###
    for (i in 1:dim(pgenes)[1]) { ###
      if (pgenes[i,2]=="") { ###
        pgenes[i,2]=pgenes[i,1] ###
      }
    }

    pgenes=pgenes[order(match(pgenes[,1], data.plot$features.plot)),] ###
    pgenes=pgenes[,2] ###
  ...
  if (!split.colors) {
    plot <- plot + guides(color = guide_colorbar(title = 'Avg.\nexpression'))
  }
  if (!is.null(gnames)){ ###
